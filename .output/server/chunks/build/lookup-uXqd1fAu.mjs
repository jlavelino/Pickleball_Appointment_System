...
import { _ as _export_sfc, u as useHead, c as useSupabase } from './server.mjs';
import '../_/nitro.mjs';
import 'node:http';
import 'node:https';
import 'node:events';
      ...
      if (error.value) {
        ...
        ssrRenderList(results.value, (b) => {
          ...
          if (b.paddles && b.paddles.length > 0) {
            _push(`<div class="flex justify-between items-start text-[13px] pt-2 border-t border-[var(--line)]/40" data-v-c0cb94d3><div data-v-c0cb94d3><div class="font-medium text-[var(--ink)]" data-v-c0cb94d3>${ssrInterpolate(b.paddles[0].quantity)} \xD7 ${ssrInterpolate(b.paddles[0].name)}</div><div class="text-[12px] text-[var(--ink-soft)] mt-0.5" data-v-c0cb94d3>`);
            if (b.paddles.length > 1) {
              _push(`<span data-v-c0cb94d3>${ssrInterpolate(b.paddles.slice(1).map((p) => `${p.quantity} \xD7 ${p.name}`).join(", "))} \xB7 </span>`);
            } else {
              _push(`<!---->`);
            }
          ...
          if (b.food_items && b.food_items.length > 0) {
            _push(`<div class="flex justify-between items-start text-[13px] pt-2 border-t border-[var(--line)]/40" data-v-c0cb94d3><div data-v-c0cb94d3><div class="font-medium text-[var(--ink)]" data-v-c0cb94d3>${ssrInterpolate(b.food_items[0].quantity)} \xD7 ${ssrInterpolate(b.food_items[0].name)}</div>`);
            if (b.food_items.length > 1) {
              _push(`<div class="text-[12px] text-[var(--ink-soft)] mt-0.5" data-v-c0cb94d3>${ssrInterpolate(b.food_items.slice(1).map((f) => `${f.quantity} \xD7 ${f.name}`).join(", "))}</div>`);
            } else {
              _push(`<!---->`);
            }
...
//# sourceMappingURL=lookup-uXqd1fAu.mjs.map
