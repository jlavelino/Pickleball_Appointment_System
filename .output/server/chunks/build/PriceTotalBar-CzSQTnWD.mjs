  ...
  setup(__props) {
    ...
    const breakdownText = computed(() => {
      ...
      if (courtCount > 0 && hours > 0) {
        parts.push(`${courtCount} court${courtCount > 1 ? "s" : ""} \xD7 ${hours} hr${hours > 1 ? "s" : ""}`);
      }
      if (store.paddleCount > 0) {
        if (hours > 1) {
          parts.push(`${store.paddleCount} paddle${store.paddleCount > 1 ? "s" : ""} \xD7 ${hours} hrs`);
        } else {
      ...
      return parts.length ? parts.join(" \xB7 ") : "Select courts & times";
    });
...
export { PriceTotalBar as P };
//# sourceMappingURL=PriceTotalBar-CzSQTnWD.mjs.map
